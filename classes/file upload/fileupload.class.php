...
class FileUploadHandler extends GenerateURL
{
    ...
    private function dbOperationPerUpload($DB_OPERATION_KEY,$o,$ProjectOperations,$name,$size,$source,$destination,$quality,$count_pictures,$count_documents,$count_files){
        switch($DB_OPERATION_KEY){
            case 1:
                if($this->compressImage($source,$destination,$quality)){
                    if($ProjectOperations->add_attachment($o))
                        $count_pictures+=1;
                }
                break;
            case 2:
            ...
            case 3:
                ...
                break;
            default:
                header("location: ../../superAdmin/dshCreateProject");
        }
    }
    public function filesPushRemote($FILES,$FILES_ARR,$FILES_KEY,$DB_OPERATION_KEY,$o,$ProjectOperations){
        $i =0;
        $count_pictures =0;
        $count_documents =0;
        $count_files =0;
        $store_url = str_ireplace("./","../","./uploads/");
        ...
        if($num_files>0){
            for($i;$i<=$max;$i++){
                ...
                $quality = 75;
                $o["file_name"] = $name;
                $o["file_type"] = 1; /*images*/
                $o["file_size"] = $size;
                $this->dbOperationPerUpload($DB_OPERATION_KEY,$o,$ProjectOperations,$name,$size,$source,$destination,$quality,$count_pictures,$count_documents,$count_files);
            }
            return true;
        }
    }
